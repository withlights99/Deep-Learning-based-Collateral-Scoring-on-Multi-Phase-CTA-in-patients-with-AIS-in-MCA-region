...
def crea_dict(data_path,name_scores):
    ...
    datadict = [{'image1': image1,'image2': image2,'image3': image3,'name':name,'c_label': c_label} for image1,image2,image3,name,c_label in zip(dict1,dict2,dict3,name_dict,c_score)]
    return datadict
...
def cal_metrics(confusion_matrix):
    ...
    for i in range(n_classes):
        ...
        TN = ALL - TP - FP - FN
        metrics_result['spec_{}'.format(i)]=TN/(TN+FP)
        # metrics_result.append([TP/(TP+FP), TP/(TP+FN), TN/(TN+FP)])
        metrics_result['spec_macro'] += metrics_result['spec_{}'.format(i)]
    metrics_result['spec_macro']= metrics_result['spec_macro']/n_classes
    print(metrics_result)
    return metrics_result['spec_macro']
...
class variants_init():
    ...
    def var_updata(self,loss,output1,output2,b_x_size,b_y1,b_y2,loss1,loss2,name):
        ...
        if flag == 'train':
            ...
            if np.max(self.test_acc1_all) > self.best_test_acc1 :
                if len(self.test_acc1_all)>2:
                    ...
                    self.best_test_y1_score = variants.y1_score
                    self.best_test_name = variants.name
                    mod = 1
            ...
            # if np.max(self.test_f1_c_all) > self.best_f1_c:
            #     self.best_f1_c = np.max(self.test_f1_c_all)
            #     self.best_test_y1_true = variants.y1_true
            #     self.best_test_y1_pred = variants.y1_pred
            #     self.best_test_y1_score = variants.y1_score
